...
class GMP:
    ...
    def gen_gmp_model(self, ts_d):
        ...
        ## final cons
        content += self.final_constrains(mode=3)
        ## initial cons
        # content+=self.initial_constrains([0,3,4,7,8,11,12,15])
        content += self.initial_constrains([0,2])
        ## weight
        content += self.gen_weight_constraints()
        testdegree = "{:b}".format(ts_d)

        testdegree = testdegree.zfill(self.field)
        self.result_CVC_file=f"{self.cipher_name}_r{self.rounds}_f{self.field}_testdegree{ts_d}_p_(0,2)_c_3.cvc"
    ...
    def solve(self):
        ...
        stp_parameters = f"{PATH_STP} {self.filename} --CVC --cryptominisat --threads 8"
